...
from app.prompts.extract_data_cv_prompts import SKILLS_PROCESSING_PROMPT, KEYWORDS_PROCESSING_PROMPT, NORMALIZE_SKILLS_PROMPT
from app.prompts.extract_data_vacancy_prompts import EXTRACT_VACANCY_PROMPT
...
async def extract_skills_from_cv(llm: ChatOpenAI, text: str) -> list[str]:
...
async def extract_skills_from_vacancy(llm: ChatOpenAI, vacancy: str) -> list[str]:
    try:

        result = llm.invoke(
            [
                SystemMessage(content=EXTRACT_VACANCY_PROMPT),
                HumanMessage(content=vacancy),
            ]
        )
